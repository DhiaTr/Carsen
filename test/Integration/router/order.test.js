...
describe('/api/orders', () => {
    ...
    describe('PUT /:id', () => {
        ...
        it('should return 400 if invalid data given', async () => {
            ...
            await order.save();
            const result = await request(server).put('/api/orders/' + order._id).set('x-auth-token', token).send({});
            expect(result.status).toBe(400);
        });
        ...
        it('should return 400 if no client with the given id exists', async () => {
            ...
            const order = new Order(orderData);
            await order.save();
            const result = await request(server).put('/api/orders/' + order._id).set('x-auth-token', token).send(orderData);
            expect(result.status).toBe(400);
        });
        ...
        it('should save the changes if valid request', async () => {
            ...
            await request(server).put('/api/orders/' + order._id).set('x-auth-token', token).send(orderData);
    ...
    describe('DELETE /:id', () => {
        ...
        it('should return 401 if user not logged in', async () => {
            const result = await request(server).delete('/api/orders/1').send();
            expect(result.status).toBe(401);
        });

        it('should return 400 if invalid token given', async () => {
            const result = await request(server).delete('/api/orders/1').set('x-auth-token', null).send();
            expect(result.status).toBe(400);
        });

        it('should return 403 if user not admin', async () => {
            const token = new Agent().generateAuthToken();
            const result = await request(server).delete('/api/orders/1').set('x-auth-token', token).send();
        ...
        it('should return 404 if no order with the given id exists', async () => {
            const token = new Agent({ isAdmin: true }).generateAuthToken();
            const id = mongoose.Types.ObjectId();
            const result = await request(server).delete('/api/orders/' + id).set('x-auth-token', token).send();
            expect(result.status).toBe(404);
        });
        ...
        it('should return 200 valid request', async () => {
            ...
            await order.save();
            const result = await request(server).delete('/api/orders/' + order._id).set('x-auth-token', token).send();
            expect(result.status).toBe(200);
        });
        ...
        it('should delete the order valid request', async () => {
            ...
            await order.save();
            await request(server).delete('/api/orders/' + order._id).set('x-auth-token', token).send();
